...
public sealed class GitHubRepoAssetStorage : IAssetStorage, IGitHubRepoStorage
{
    ...
    public async Task<StoredAssetObject> OverwriteAsync(
        Stream content,
        string storageKey,
        StoreAssetRequest request,
        CancellationToken cancellationToken = default)
    {
        ...
        var relativePath = PathResolver.ToRelativePath(normalizedStorageKey);
        var upsert = await UpsertFileAsync(
            content,
            new GitHubRepoUpsertFileRequest(
                RelativePath: relativePath,
    ...
    public async Task<Stream?> OpenReadAsync(string storageKey, CancellationToken cancellationToken = default)
    {
        ...
        {
            // 公开仓库且未使用 token 时优先走 raw 地址，避免 contents API 的 base64 包装和额外限流成本。
        ...
        // raw 直读失败后回退到 contents API，既能兼容私有仓库，也能兜底处理 raw URL 不可用的情况。
        return await DownloadViaContentsApiAsync(normalizedStorageKey, cancellationToken);
    }

    public async Task DeleteAsync(DeleteStoredAssetRequest request, CancellationToken cancellationToken = default)
    {
        var storageKey = PathResolver.NormalizeStorageKey(request.StorageKey);
        var metadata = await TryGetFileMetadataAsync(storageKey, cancellationToken);
        if (metadata is null)
        {
            throw new NotFoundException(
                "storage_provider_target_not_found",
                $"Target '{PathResolver.ToRelativePath(storageKey)}' was not found in github-repo provider.");
        }

        if (string.IsNullOrWhiteSpace(metadata.Sha))
        {
            throw new ValidationException(
                "storage_provider_delete_result_invalid",
                "github-repo delete target metadata did not return file sha.");
        }

        var relativePath = PathResolver.ToRelativePath(storageKey);
        var requestBody = new GitHubDeleteContentRequest(
        ...
        var requestMessage = CreateRequest(HttpMethod.Delete, PathResolver.BuildContentsApiUri(storageKey), includeToken: true);
        ...
        if (response.StatusCode == HttpStatusCode.NotFound)
        {
            throw new NotFoundException(
                "storage_provider_target_not_found",
                $"Target '{relativePath}' was not found in github-repo provider.");
        }
        ...
        if (response.StatusCode is not HttpStatusCode.OK and not HttpStatusCode.NoContent)
        {
            await ThrowForFailedResponseAsync(
                response,
                "storage_provider_delete_failed",
                $"Failed to delete '{relativePath}' in github-repo provider.",
                cancellationToken);
        }
    }
    ...
    public Task<string?> GetPublicUrlAsync(string storageKey, CancellationToken cancellationToken = default)
    {
        ...
        {
            // private-token 模式下即使知道 raw 路径，也不把可外链地址暴露给上层。
    ...
    {
        // expectedSha 充当乐观并发保护，避免多人编辑同一路径时后写覆盖先写。
    ...
    private async Task<Stream?> DownloadViaContentsApiAsync(string storageKey, CancellationToken cancellationToken)
    {
        ...
        // contents API 可能直接内嵌 base64，也可能只返回 download_url，这里同时兼容两种响应形态。
    ...
    private HttpRequestMessage CreateRequest(HttpMethod method, Uri uri, bool includeToken)
    {
        ...
        {
            // raw 公网地址不主动附带 token，避免把私密凭证带到非 API 域名的下载请求上。
    ...
    {
        // 只有 public-only 且未配置 token 时才默认走 raw 读链路；
        // 一旦涉及私有读取或受控访问，统一退回 contents API。
    ...
    private sealed record GitHubDeleteContentRequest(
        string Message,
        string Sha,
        string Branch);
