    ...
    Microsoft.Extensions.Options.IOptions<StorageOptions> storageOptions) : IAssetStorage
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var physicalPath = ResolvePhysicalPath(rootPath, normalizedStorageKey);
        Directory.CreateDirectory(Path.GetDirectoryName(physicalPath)!);

        var tempPath = $"{physicalPath}.{Guid.CreateVersion7():N}.tmp";
        ...
        return new StoredAssetObject(
            ...
            PublicUrl: BuildPublicUrl(normalizedStorageKey),
    ...
    {
        var rootPath = Path.GetFullPath(_localStorageOptions.RootPath);
        var physicalPath = ResolvePhysicalPath(rootPath, request.StorageKey);
        if (File.Exists(physicalPath))
        {
