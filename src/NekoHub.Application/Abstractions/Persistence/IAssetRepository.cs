...
public interface IAssetRepository
{
    ...
    Task<Asset?> GetPublicByIdAsync(Guid assetId, CancellationToken cancellationToken = default);

    Task<Asset?> GetByStorageKeyAsync(string storageKey, CancellationToken cancellationToken = default);

    Task<bool> AnyByStorageProviderProfileIdAsync(Guid storageProviderProfileId, CancellationToken cancellationToken = default);

    Task<PagedResult<Asset>> GetPagedAsync(GetAssetsPagedQuery query, CancellationToken cancellationToken = default);

    Task<PagedResult<Asset>> GetPublicPagedAsync(GetAssetsPagedQuery query, CancellationToken cancellationToken = default);
