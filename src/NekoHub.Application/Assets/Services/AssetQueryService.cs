    ...
    IAssetSkillExecutionRepository assetSkillExecutionRepository) : IAssetQueryService
{
    ...
    public async Task<PublicAssetQueryDto> GetPublicByIdAsync(Guid assetId, CancellationToken cancellationToken = default)
    {
        var asset = await assetRepository.GetPublicByIdAsync(assetId, cancellationToken);
        if (asset is null)
        {
            throw new NotFoundException("asset_not_found", $"Asset '{assetId}' was not found.");
        }
        ...
        var derivatives = await assetDerivativeRepository.GetBySourceAssetIdAsync(asset.Id, cancellationToken);
        return ToPublicDto(asset, derivatives);
    }

    public async Task<PublicAssetPagedQueryDto> GetPublicPagedAsync(
        GetAssetsPagedQuery query,
        CancellationToken cancellationToken = default)
    {
        ValidatePaging(query);

        var normalizedQuery = NormalizeQuery(query);
        var paged = await assetRepository.GetPublicPagedAsync(normalizedQuery, cancellationToken);
        var items = paged.Items.Select(ToPublicListItemDto).ToList();

        return new PublicAssetPagedQueryDto(items, paged.Page, paged.PageSize, paged.Total);
    }
    ...
    private static PublicAssetListItemQueryDto ToPublicListItemDto(Asset asset)
    {
        return new PublicAssetListItemQueryDto(
            Id: asset.Id,
            Type: asset.Type,
            OriginalFileName: asset.OriginalFileName,
            ContentType: asset.ContentType,
    ...
    private static PublicAssetQueryDto ToPublicDto(
        Asset asset,
        IReadOnlyList<AssetDerivative> derivatives)
    {
        return new PublicAssetQueryDto(
            Id: asset.Id,
            Type: asset.Type,
            OriginalFileName: asset.OriginalFileName,
            ContentType: asset.ContentType,
            ...
            Derivatives: derivatives
                .OrderBy(static derivative => derivative.CreatedAtUtc)
                .Select(static derivative => new PublicAssetDerivativeSummaryQueryDto(
                    Kind: derivative.Kind,
                    ContentType: derivative.ContentType,
                    Extension: derivative.Extension,
                    Size: derivative.Size,
