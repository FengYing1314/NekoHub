    ...
    IAssetProcessingQueue assetProcessingQueue) : IAssetCommandService
{
    ...
    public async Task<AssetDto> UploadAsync(UploadAssetCommand command, CancellationToken cancellationToken = default)
    {
        ...
        // 元数据提取和校验和计算都会重复回卷流，因此这里先把非可寻址流规整成可 seek 的实现。
        ...
        uploadStream.Position = 0;
        // 写入目标既可能来自显式 profile，也可能回落到当前默认写入 profile；
        // 这里统一通过 lease 持有运行时 storage，避免上层关心 provider 的释放细节。
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // 批量删除采用“尽量继续”的语义：单个资源不存在只记结果，不中断整批处理。
    ...
    private static string ResolveCommitMessage(string? commitMessage)
    {
