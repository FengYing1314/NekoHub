    ...
    ILogger<McpServer> logger) : IMcpServer
{
    ...
    public async Task<McpHttpResponse> HandlePostAsync(HttpRequest request, CancellationToken cancellationToken)
    {
        ...
        using (payloadDocument)
        {
            ...
            {
                // 当前实现只接受单条 JSON-RPC envelope，避免批量请求下的关联和局部失败语义复杂化。
                return BuildErrorResponse(
                    StatusCodes.Status400BadRequest,
                    null,
                    InvalidRequestCode,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            // 服务器端当前没有需要消费的 client->server 响应帧，直接按已接受处理。
            return new McpHttpResponse(StatusCodes.Status202Accepted);
        }

        if (envelope.IsNotification)
        {
            // 通知型消息同样不回 body，维持 MCP/JSON-RPC 的 fire-and-forget 语义。
            return new McpHttpResponse(StatusCodes.Status202Accepted);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            // 领域级错误通过 tool result 返回给 MCP 客户端，而不是升级成 transport 级 5xx。
            return BuildSuccessResponse(
                requestId,
        ...
        out string? errorMessage)
    {
        ...
        // header 存在时必须是服务端支持的版本；协商的降级只发生在 initialize 的 params.protocolVersion 上。
    ...
    private static bool IsOriginAllowed(HttpRequest request)
    {
        ...
        // 反向代理场景下优先使用 forwarded headers，保持同源判断与外部暴露地址一致。
