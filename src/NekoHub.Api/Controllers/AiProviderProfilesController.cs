...
{
    [HttpGet]
    [Authorize(Policy = PermissionCatalog.AiProvidersRead)]
    ...
    [HttpPost]
    [Authorize(Policy = PermissionCatalog.AiProvidersCreate)]
    ...
    [HttpPatch("{id:guid}")]
    [Authorize(Policy = PermissionCatalog.AiProvidersUpdate)]
    [ProducesResponseType(typeof(ApiResponse<AiProviderProfileResponse>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ApiErrorResponse), StatusCodes.Status400BadRequest)]
    [ProducesResponseType(typeof(ApiErrorResponse), StatusCodes.Status401Unauthorized)]
    ...
    [HttpPost("test")]
    [Authorize(Policy = PermissionCatalog.AiProvidersUpdate)]
    [ProducesResponseType(typeof(ApiResponse<AiProviderProfileTestResponse>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ApiErrorResponse), StatusCodes.Status400BadRequest)]
    [ProducesResponseType(typeof(ApiErrorResponse), StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(typeof(ApiErrorResponse), StatusCodes.Status404NotFound)]
    [ProducesResponseType(typeof(ApiErrorResponse), StatusCodes.Status500InternalServerError)]
    [EndpointSummary("Test AI provider profile")]
    public async Task<IActionResult> TestAsync(
        [FromBody] TestAiProviderProfileRequest request,
        CancellationToken cancellationToken)
    {
        var result = await aiProviderProfileService.TestAsync(
            new TestAiProviderProfileCommand(
    ...
    [HttpDelete("{id:guid}")]
    [Authorize(Policy = PermissionCatalog.AiProvidersDelete)]
    ...
    private static AiProviderProfileTestResponse ToResponse(AiProviderProfileTestResultDto dto)
    {
        return new AiProviderProfileTestResponse(
            Succeeded: dto.Succeeded,
            Caption: dto.Caption,
            ResolvedModelName: dto.ResolvedModelName,
            ResolvedApiBaseUrl: dto.ResolvedApiBaseUrl,
