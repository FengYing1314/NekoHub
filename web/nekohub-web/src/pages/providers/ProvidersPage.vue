...
import {
  ...
  createDefaultGitHubReleasesFormValues,
  mapGitHubReleasesSummaryToForm,
  type GitHubReleasesReleaseTagMode,
  type GitHubReleasesVisibilityPolicy,
...
const defaultGitHubReleasesValues = createDefaultGitHubReleasesFormValues();
...
const canCreateProviders = computed(() => can(PERMISSIONS.providersCreate));
const canUpdateProviders = computed(() => can(PERMISSIONS.providersUpdate));
const canDeleteProviders = computed(() => can(PERMISSIONS.providersDelete));
const isProvidersReadOnly = computed(() => (
  !canCreateProviders.value
  && !canUpdateProviders.value
  && !canDeleteProviders.value
));
...
const profileForm = reactive({
  ...
  githubToken: '',
  githubReleasesOwner: defaultGitHubReleasesValues.owner,
  githubReleasesRepo: defaultGitHubReleasesValues.repo,
  githubReleasesReleaseTagMode: defaultGitHubReleasesValues.releaseTagMode as GitHubReleasesReleaseTagMode,
  githubReleasesFixedTag: defaultGitHubReleasesValues.fixedTag,
  githubReleasesAssetPathPrefix: defaultGitHubReleasesValues.assetPathPrefix,
  githubReleasesVisibilityPolicy: defaultGitHubReleasesValues.visibilityPolicy as GitHubReleasesVisibilityPolicy,
  githubReleasesToken: defaultGitHubReleasesValues.token,
});
...
const providerTypeOptions = computed<SelectOption[]>(() => [
  ...
  {
    label: t('settings.storage.providers.githubReleases'),
    value: 'github-releases',
  },
]);

// 表单层允许维护 github-releases profile；
// 是否能作为实际运行时写入 provider，以服务端 runtime factory 的能力为准。
const supportedFormProviderTypes = new Set<StorageProviderType>(['local', 's3-compatible', 'github-repo', 'github-releases']);
...
const githubReleasesTagModeOptions = computed<SelectOption[]>(() => [
  {
    label: t('settings.storage.form.githubReleases.releaseTagModes.latest'),
    value: 'latest',
  },
  {
    label: t('settings.storage.form.githubReleases.releaseTagModes.fixed'),
    value: 'fixed',
  },
]);

const githubReleasesVisibilityPolicyOptions = computed<SelectOption[]>(() => [
  {
    label: t('settings.storage.form.githubReleases.visibilityPolicies.publicFirst'),
    value: 'public-first',
  },
  {
    label: t('settings.storage.form.githubReleases.visibilityPolicies.publicOnly'),
    value: 'public-only',
  },
]);
  ...
  },
  {
    ...
    render: (profile) => {
      ...
      return h(
        ...
        [
          h('div', { class: 'provider-type-name' }, getProviderTypeLabel(profile.providerType)),
          badges.length > 0
            ? h(
              NSpace,
              {
              ...
              default: () => [
                ...
                h(
                  NButton,
                  {
                    size: 'small',
                    quaternary: true,
                    disabled: rowDisabled || !supportsEdit || !canUpdateProviders.value,
                ...
                !profile.isDefault
                  ? h(
                    NButton,
                    {
                      ...
                      disabled: rowDisabled || !setDefaultState.canSetDefault || !canUpdateProviders.value,
                ...
                h(
                  NButton,
                  {
                    ...
                    disabled: rowDisabled || !canDeleteProviders.value,
...
const isGitHubReleasesSelected = computed(() => profileForm.providerType === 'github-releases');
...
function getRuntimeSelectionSourceLabel(selectionSource: string): string {
  switch (selectionSource) {
    case 'configuration':
      return t('settings.storage.alignment.selectionSources.configuration');
    case 'database-default-profile':
      return t('settings.storage.alignment.selectionSources.databaseDefaultProfile');
    default:
      return t('settings.storage.alignment.selectionSources.unknown');
  }
}
...
function formatConfigurationSummary(summary: StorageProviderConfigurationSummaryResponse): string {
  ...
  if (summary.providerName) {
    entries.push(`提供方：${summary.providerName}`);
  }

  if (summary.rootPath) {
    entries.push(`根路径：${summary.rootPath}`);
  }

  if (summary.endpointHost) {
    entries.push(`终结点主机：${summary.endpointHost}`);
  }

  if (summary.bucketOrContainer) {
    entries.push(`存储桶或容器：${summary.bucketOrContainer}`);
  }

  if (summary.region) {
    entries.push(`区域：${summary.region}`);
  }

  if (summary.publicBaseUrl) {
    entries.push(`公开访问地址：${summary.publicBaseUrl}`);
  }

  if (summary.forcePathStyle !== null) {
    entries.push(`路径风格：${summary.forcePathStyle ? '启用' : '关闭'}`);
  }

  if (summary.owner) {
    entries.push(`仓库所有者：${summary.owner}`);
  }

  if (summary.repository) {
    entries.push(`仓库名称：${summary.repository}`);
  }

  if (summary.reference) {
    entries.push(`引用版本：${summary.reference}`);
  }

  if (summary.releaseTagMode) {
    entries.push(`发布标签模式：${summary.releaseTagMode}`);
  }

  if (summary.fixedTag) {
    entries.push(`固定标签：${summary.fixedTag}`);
  }

  if (summary.basePath) {
    entries.push(`基础路径：${summary.basePath}`);
  }

  if (summary.assetPathPrefix) {
    entries.push(`资源路径前缀：${summary.assetPathPrefix}`);
  }

  if (summary.visibilityPolicy) {
    entries.push(`可见性策略：${summary.visibilityPolicy}`);
  }

  if (summary.apiBaseUrl) {
    entries.push(`接口基地址：${summary.apiBaseUrl}`);
  }

  if (summary.rawBaseUrl) {
    entries.push(`原始内容地址：${summary.rawBaseUrl}`);
  }
...
function resetProfileForm(): void {
  ...
  profileForm.githubReleasesOwner = defaultGitHubReleasesValues.owner;
  profileForm.githubReleasesRepo = defaultGitHubReleasesValues.repo;
  profileForm.githubReleasesReleaseTagMode = defaultGitHubReleasesValues.releaseTagMode;
  profileForm.githubReleasesFixedTag = defaultGitHubReleasesValues.fixedTag;
  profileForm.githubReleasesAssetPathPrefix = defaultGitHubReleasesValues.assetPathPrefix;
  profileForm.githubReleasesVisibilityPolicy = defaultGitHubReleasesValues.visibilityPolicy;
  profileForm.githubReleasesToken = defaultGitHubReleasesValues.token;
...
function openEditProfileModal(profile: StorageProviderProfileResponse): void {
  ...
  if (profile.providerType === 'github-releases') {
    const mappedValues = mapGitHubReleasesSummaryToForm(profile.configurationSummary);
    profileForm.githubReleasesOwner = mappedValues.owner;
    profileForm.githubReleasesRepo = mappedValues.repo;
    profileForm.githubReleasesReleaseTagMode = mappedValues.releaseTagMode;
    profileForm.githubReleasesFixedTag = mappedValues.fixedTag;
    profileForm.githubReleasesAssetPathPrefix = mappedValues.assetPathPrefix;
    profileForm.githubReleasesVisibilityPolicy = mappedValues.visibilityPolicy;
    profileForm.githubReleasesToken = '';
  }
...
function applySelectedS3Preset(): void {
  ...
  // 预设只填充当前仍为空的字段，避免覆盖用户已经手工调整过的 endpoint/region/path-style。
...
function validateProfileForm(): string | null {
  ...
  if (profileForm.providerType === 'github-releases') {
    const errorKey = validateGitHubReleasesForm({
      owner: profileForm.githubReleasesOwner,
      repo: profileForm.githubReleasesRepo,
      releaseTagMode: profileForm.githubReleasesReleaseTagMode,
      fixedTag: profileForm.githubReleasesFixedTag,
      assetPathPrefix: profileForm.githubReleasesAssetPathPrefix,
      visibilityPolicy: profileForm.githubReleasesVisibilityPolicy,
      token: profileForm.githubReleasesToken,
    });
...
function isValidGitHubPathPrefix(value: string): boolean {
  // basePath 会直接参与远端仓库路径拼接，这里提前剔除空段和目录回退片段。
...
function buildProfileConfiguration(providerType: StorageProviderType): Record<string, unknown> {
  ...
  if (providerType === 'github-releases') {
    return buildGitHubReleasesConfiguration({
      owner: profileForm.githubReleasesOwner,
      repo: profileForm.githubReleasesRepo,
      releaseTagMode: profileForm.githubReleasesReleaseTagMode,
      fixedTag: profileForm.githubReleasesFixedTag,
  ...
  throw new Error(`当前表单暂不支持这种存储类型：${providerType}`);
}

async function submitProfileModal(): Promise<void> {
  if (!isEditMode.value && !canCreateProviders.value) {
    message.warning(t('auth.permissionDenied'));
    return;
  }

  if (isEditMode.value && !canUpdateProviders.value) {
    message.warning(t('auth.permissionDenied'));
    return;
  }
  ...
  try {
    if (!isEditMode.value) {
      ...
      if (profileForm.providerType === 'github-repo') {
        const token = profileForm.githubToken.trim();
        if (token) {
          // 编辑态不强制回传 token，避免用户未改密钥时把已掩码保存的 secret 误清空。
          createRequest.secretConfiguration = buildGitHubRepoSecretConfiguration();
        }
      }

      if (profileForm.providerType === 'github-releases') {
        const secretConfiguration = buildGitHubReleasesSecretConfiguration({
          owner: profileForm.githubReleasesOwner,
          repo: profileForm.githubReleasesRepo,
          releaseTagMode: profileForm.githubReleasesReleaseTagMode,
          fixedTag: profileForm.githubReleasesFixedTag,
        ...
        if (secretConfiguration) {
          createRequest.secretConfiguration = secretConfiguration;
        }
      }
      ...
      if (profileForm.providerType === 'github-releases') {
        ...
        if (secretConfiguration) {
          updateRequest.secretConfiguration = secretConfiguration;
        }
      }
...
<template>
  <div>
    <page-header :title="t('providers.title')" :description="t('providers.description')">
      <template #actions>
        <n-space :wrap="true">
          ...
          <n-button
            v-if="canCreateProviders"
            size="small"
            type="primary"
            secondary
            :disabled="storageLoading || profileModalSubmitting"
    ...
    <n-alert v-if="isProvidersReadOnly" type="info" style="margin-bottom: 16px">
          ...
          >
            ...
            <n-space wrap :size="8" class="alignment-meta">
                <n-tag size="small" type="info" :bordered="false">
                  ...
                  {{ getRuntimeSelectionSourceLabel(storageOverview.alignment.runtimeSelectionSource) }}
                </n-tag>
              </n-space>
            </n-alert>
            ...
            <div class="providers-section">
              ...
              <n-descriptions bordered size="small" :column="isMobile ? 1 : 2">
                <n-descriptions-item :label="t('settings.storage.runtime.providerType')">
                  {{ getProviderTypeLabel(storageOverview.runtime.providerType) }}
                </n-descriptions-item>
            ...
            <div class="providers-section">
              ...
              >
                ...
                <n-descriptions-item :label="t('settings.storage.defaultProfile.providerType')">
                  {{ getProviderTypeLabel(storageOverview.defaultProfile.providerType) }}
                </n-descriptions-item>
    ...
    >
      <n-form label-placement="top" class="profile-form">
          ...
          <section v-else-if="isGitHubReleasesSelected" class="profile-form-section">
            <h4 class="profile-form-section__title">{{ t('providers.modal.sections.githubReleases') }}</h4>
            ...
            <div class="profile-provider-meta">
              ...
              <div class="form-hint">{{ t('providers.githubReleases.formHint') }}</div>
            </div>

            <div class="profile-form-grid">
              <n-form-item :label="t('settings.storage.form.github.owner')">
                <n-input
                  v-model:value="profileForm.githubReleasesOwner"
              ...
              <n-form-item :label="t('settings.storage.form.github.repo')">
                <n-input
                  v-model:value="profileForm.githubReleasesRepo"
                  :placeholder="t('settings.storage.form.github.repoPlaceholder')"
                />
              </n-form-item>

              <n-form-item :label="t('settings.storage.form.githubReleases.releaseTagMode')">
                <n-select
                  v-model:value="profileForm.githubReleasesReleaseTagMode"
                  :options="githubReleasesTagModeOptions"
                />
              </n-form-item>

              <n-form-item :label="t('settings.storage.form.githubReleases.fixedTag')">
                <n-input
                  v-model:value="profileForm.githubReleasesFixedTag"
                  :disabled="profileForm.githubReleasesReleaseTagMode !== 'fixed'"
                  :placeholder="t('settings.storage.form.githubReleases.fixedTagPlaceholder')"
                />
              </n-form-item>

              <n-form-item :label="t('settings.storage.form.githubReleases.assetPathPrefix')">
                <n-input
                  v-model:value="profileForm.githubReleasesAssetPathPrefix"
                  :placeholder="t('settings.storage.form.githubReleases.assetPathPrefixPlaceholder')"
                />
              </n-form-item>

              <n-form-item :label="t('settings.storage.form.githubReleases.visibilityPolicy')">
                <n-select
                  v-model:value="profileForm.githubReleasesVisibilityPolicy"
                  :options="githubReleasesVisibilityPolicyOptions"
                />
              </n-form-item>
            </div>

            <n-alert v-if="isEditMode" type="info" :show-icon="false" class="form-alert">
              {{ t('settings.storage.form.githubReleases.secretUpdateHint') }}
            </n-alert>

            <div class="profile-form-grid">
              <n-form-item :label="t('settings.storage.form.githubReleases.token')">
                <n-input
                  v-model:value="profileForm.githubReleasesToken"
                  type="password"
                  show-password-on="mousedown"
                  :placeholder="t('settings.storage.form.githubReleases.tokenPlaceholder')"
                />
              </n-form-item>
            </div>
          </section>
