...
const canUpdateSettings = computed(() => can(PERMISSIONS.settingsUpdate));
...
<template>
  <div>
    ...
    <n-alert v-if="!canUpdateSettings" type="info" style="margin-bottom: 16px">
      {{ t('common.readOnlyNotice') }}
    </n-alert>

    <n-card class="section-card settings-card">
      <n-form label-placement="top" :model="formModel">
        ...
        <div class="settings-actions">
          <n-button type="primary" :disabled="!canUpdateSettings" @click="handleSave">{{ t('common.save') }}</n-button>
        </div>
      </n-form>
    </n-card>

    <n-card class="section-card settings-card" :title="t('settings.providersEntry.title')">
