  ...
  },
  {
    ...
    render: (item) => h(
      ...
      },
      {
        default: () => (
          item.isDirectory
            ? t('settings.storage.githubRepo.filters.types.dir')
            : t('settings.storage.githubRepo.filters.types.file')
        ),
      },
    ),
  },
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      if (typeof result !== 'string') {
        reject(new Error('读取文件失败，浏览器返回了无法识别的数据。'));
        return;
      }

      const commaIndex = result.indexOf(',');
      if (commaIndex < 0) {
        reject(new Error('读取文件失败，生成的数据格式无效。'));
        return;
      }
    ...
    reader.onerror = () => reject(reader.error ?? new Error('读取文件失败，请重新选择文件后再试。'));
