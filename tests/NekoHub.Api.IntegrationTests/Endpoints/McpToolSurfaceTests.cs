...
public class McpToolSurfaceTests : IntegrationTestBase, IClassFixture<NekoHubApiKeyApplicationFactory>
{
    private const string ProtocolVersion = "2025-11-25";

    public McpToolSurfaceTests(NekoHubApiKeyApplicationFactory factory) : base(factory)
    {
        // MCP 端点要求 API key，这里显式覆盖请求头，避免误走默认 JWT 测试习惯。
    ...
    public async Task Mcp_Asset_Tools_Should_Reuse_Asset_Read_Model_And_Hide_Storage_Internals()
    {
        ...
        var assetStructuredContent = await WaitForAssetToolContentAsync(
            assetId,
            11,
            asset => asset["derivatives"]?.AsArray()?.Count > 0);
        ...
        assetStructuredContent["structuredResults"]?.AsArray().Should().NotBeNull();
    ...
    {
        var assetId = await UploadTestPngAsync("mcp-skill-target.png", runEnrichment: false);
        ...
        asset["derivatives"]?.AsArray().Should().NotBeNull();
        asset["structuredResults"]?.AsArray().Should().NotBeNull();

        var enrichedAsset = await WaitForAssetToolContentAsync(
            assetId,
            170,
            item => item["derivatives"]?.AsArray()?.Count > 0);
        enrichedAsset["derivatives"]?.AsArray().Should().NotBeEmpty();
        enrichedAsset["structuredResults"]?.AsArray().Should().NotBeNull();
    }
    ...
    public async Task Mcp_Upload_Asset_Should_Create_Asset_With_Derivatives_And_Structured_Results()
    {
        ...
        structuredContent["derivatives"]?.AsArray().Should().NotBeNull();
        structuredContent["structuredResults"]?.AsArray().Should().NotBeNull();
    ...
    public async Task Mcp_Private_Asset_Tools_Should_Hide_PublicUrl_And_Block_Content_Url()
    {
        ...
        var privateAsset = await WaitForAssetToolContentAsync(
            assetId,
            46,
            item => item["isPublic"]?.GetValue<bool>() == false
                && item["derivatives"]?.AsArray()?.Count > 0);
        privateAsset["publicUrl"].Should().BeNull();
        privateAsset["derivatives"]?.AsArray()
    ...
    public async Task Mcp_Batch_Delete_Assets_Should_Report_Deleted_And_NotFound_Items()
    {
        ...
        var structuredContent = json["result"]?["structuredContent"]?.AsObject();
        structuredContent.Should().NotBeNull();
        structuredContent!.ToJsonString().Should().Contain(missingId.ToString());
    }
    ...
    private async Task<HttpResponseMessage> PostMcpAsync(object payload, string? protocolVersion = null)
    {
        ...
        // 同时声明 JSON 和 SSE 可接受，覆盖 MCP 常见的两种响应协商方式。
    ...
    private async Task<JsonObject> WaitForAssetToolContentAsync(Guid assetId, int requestId, Func<JsonObject, bool> predicate)
    {
        // MCP 读取的对象包含异步 enrichment 结果，因此这里通过 get_asset 轮询到目标状态再断言。
