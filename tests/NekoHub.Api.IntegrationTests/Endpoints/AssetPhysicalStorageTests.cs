...
public class AssetPhysicalStorageTests : IntegrationTestBase
{
    ...
    {
        // 1. 上传后物理文件存在
        var uploadResponse = await UploadTestImage(Client, "test_physical.png", "image/png", new byte[100]);
        ...
        // 删除先撤销业务访问，物理清理可能等待后台处理释放同一资产。
        (await EventuallyAsync(() => Task.FromResult(File.Exists(physicalPath)), exists => !exists))
            .Should().BeFalse("删除后的物理文件应最终由清理任务移除");
    }
    ...
    public async Task Manual_Physical_File_Deletion_Should_Not_Break_Api_Delete()
    {
        ...
        var uploadResponse = await UploadTestImage(Client, "test_manual_del.png", "image/png", new byte[50]);
        var asset = await GetResponseDataAsync<AssetResponse>(uploadResponse);
        var assetId = asset!.Id;
    ...
    [Fact]
    public async Task Delete_Should_Revoke_Access_Before_Deferred_File_Cleanup_Completes()
    {
        using var factory = new BlockingProcessingFactory();
        using var client = factory.CreateClient();
        var upload = await UploadTestImage(client, "deferred-delete.png", "image/png", new byte[100]);
        upload.StatusCode.Should().Be(HttpStatusCode.Accepted);
        var asset = (await GetResponseDataAsync<AssetResponse>(upload))!;
        var physicalPath = Path.Combine(factory.TestStoragePath, asset.StorageKey);

        // 确定性地保留 running 作业，覆盖 CI 中偶发的清理推迟窗口。
        await factory.Dispatcher.Started.Task.WaitAsync(TimeSpan.FromSeconds(10));
        try
        {
            var deleted = await client.DeleteAsync($"/api/v1/assets/{asset.Id}");
            deleted.StatusCode.Should().Be(HttpStatusCode.OK);
            File.Exists(physicalPath).Should().BeTrue("仍在处理的资产不能被即时清理抢先删除文件");
            (await client.GetAsync($"/api/v1/assets/{asset.Id}")).StatusCode.Should().Be(HttpStatusCode.NotFound);
            (await client.GetAsync($"/api/v1/assets/{asset.Id}/content")).StatusCode.Should().Be(HttpStatusCode.NotFound);
            (await client.GetAsync($"/content/{asset.StorageKey}")).StatusCode.Should().Be(HttpStatusCode.NotFound);
        }
        finally
        {
            factory.Dispatcher.Release.TrySetResult();
        }

        (await EventuallyAsync(() => Task.FromResult(File.Exists(physicalPath)), exists => !exists))
            .Should().BeFalse("处理结束后，后台应恢复并完成已持久化的文件清理");
    }

    private static async Task<HttpResponseMessage> UploadTestImage(HttpClient client, string fileName, string contentType, byte[] content)
    {
    ...
    private sealed class BlockingProcessingFactory : NekoHubApplicationFactory
    {
        public BlockingDispatcher Dispatcher { get; } = new();
    ...
    private sealed class BlockingDispatcher : IAssetProcessingDispatcher
    {
        public TaskCompletionSource Started { get; } = new(TaskCreationOptions.RunContinuationsAsynchronously);
        public TaskCompletionSource Release { get; } = new(TaskCreationOptions.RunContinuationsAsynchronously);

        public async Task DispatchAsync(AssetProcessingRequest request, CancellationToken cancellationToken = default)
        {
